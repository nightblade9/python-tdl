...
class _BaseConsole(object):
    ...
    def _normalizeCursor(self, x, y):
        ...
        while y >= height:
            if self._scrollMode == 'scroll':
                ...
                self._cursor = (0, 0)
                raise tdl.Error('Cursor has reached the end of the console')
        return (x, y)
    ...
    def set_mode(self, mode):
        ...
        @param mode: Possible settings are:
                      ...
                      - 'error' - A tdl.Error will be raised once the cursor
        ...
        if mode.lower() not in MODES:
            raise tdl.Error('mode must be one of %s, got %s' % (MODES, repr(mode)))
    ...
    def draw_str(self, x, y, string, fg=Ellipsis, bg=Ellipsis):
        ...
        def _drawStrGen(x=x, y=y, string=string, width=width, height=height):
            ...
            for char in _format_str(string):
                if y == height:
                    raise tdl.Error('End of console reached.')
...
class Console(_BaseConsole):
    ...
    def __init__(self, width, height):
        ...
        if not _rootinitialized:
            raise tdl.Error('Can not create Console instances before a call to tdl.init')
...
def init(width, height, title=None, fullscreen=False, renderer='OPENGL'):
    ...
    if renderer.upper() not in RENDERERS:
        raise tdl.Error('No such render type "%s", expected one of "%s"' % (renderer, '", "'.join(RENDERERS)))
...
def flush():
    ...
    """
    if not _rootinitialized:
        raise tdl.Error('Cannot flush without first initializing with tdl.init')
...
def set_font(path, columns=None, rows=None, columnFirst=False,
    ...
    if not _os.path.exists(path):
        raise tdl.Error('no file exists at: "%s"' % path)
    ...
    if imgSize:
        ...
        if match:
            ...
            if not (columns and rows):
                ...
                raise tdl.Error('%s has no font size in filename' % _os.path.basename(path))
    ...
    if not (columns and rows):
        # didn't auto-detect
        raise tdl.Error('Can not auto-detect the tileset of %s' % _os.path.basename(path))
...
def get_fullscreen():
    ...
    """
    if not _rootinitialized:
        raise tdl.Error('Initialize first with tdl.init')
...
def set_title(title):
    ...
    """
    if not _rootinitialized:
        raise tdl.Error('Not initilized.  Set title with tdl.init')
...
_BaseConsole.setMode = tdl.style.backport(_BaseConsole.set_mode)
_BaseConsole.setColors = tdl.style.backport(_BaseConsole.set_colors)
_BaseConsole.printStr = tdl.style.backport(_BaseConsole.print_str)
_BaseConsole.drawChar = tdl.style.backport(_BaseConsole.draw_char)
_BaseConsole.drawStr = tdl.style.backport(_BaseConsole.draw_str)
_BaseConsole.drawRect = tdl.style.backport(_BaseConsole.draw_rect)
_BaseConsole.drawFrame = tdl.style.backport(_BaseConsole.draw_frame)
_BaseConsole.getCursor = tdl.style.backport(_BaseConsole.get_cursor)
_BaseConsole.getSize = tdl.style.backport(_BaseConsole.get_size)
_BaseConsole.getChar = tdl.style.backport(_BaseConsole.get_char)

Console.getChar = tdl.style.backport(Console.get_char)

Window.drawChar = tdl.style.backport(Window.draw_char)
Window.drawRect = tdl.style.backport(Window.draw_rect)
Window.drawFrame = tdl.style.backport(Window.draw_frame)
Window.getChar = tdl.style.backport(Window.get_char)

setFont = tdl.style.backport(set_font)
getFullscreen = tdl.style.backport(get_fullscreen)
setFullscreen = tdl.style.backport(set_fullscreen)
setTitle = tdl.style.backport(set_title)
setFPS = tdl.style.backport(set_fps)
getFPS = tdl.style.backport(get_fps)
forceResolution = tdl.style.backport(force_resolution)
